...
<form method="get">
    ...
    <input type="number" min="0" name="rows" id="rows" required><br><br>

    <label>Nombre de colonnes (C) :</label>
    <input type="number" min="0" name="cols" id="cols" required><br><br>

    <label>Taille de bordure (B en pixels) :</label>
    <input type="number" min="0" name="border" id="border" required><br><br>
...
if  (isset($_GET["rows"]) && $_GET['rows']!=="" && isset($_GET["cols"]) && $_GET['cols']!=="" && isset($_GET["border"])&& $_GET['border']!=="") {
    ...
    for ($i = 0; $i < $rows; $i++) {
        echo "<tr>";
        for ($j = 0; $j < $cols; $j++) {
            echo "<td style='border: {$border}px solid black; width: 50px; height: 30px;'>[$i,$j]</td>";
        }
        echo "</tr>";
    }
